...
* --- Define Key Execution Parameters in Time Indeces -------------------------
    ...
    mSettings('schedule', 't_end') = 96; // Last time step to be included in the solve (may solve and output more time steps in case t_jump does not match)

    // Define simulation horizon and moving horizon optimization "speed"
    mSettings('schedule', 't_horizon') = 36;    // How many active time steps the solve contains (aggregation of time steps does not impact this, unless the aggregation does not match)
    mSettings('schedule', 't_jump') = 4;          // How many time steps the model rolls forward between each solve
...
* --- Define Time Step Intervals ----------------------------------------------
    ...
    mSettings('schedule', 'stepLengthInHours') = 0.25;

    // Define the time step intervals in time-steps
    mInterval('schedule', 'stepsPerInterval', 'c000') = 1;
    mInterval('schedule', 'lastStepInIntervalBlock', 'c000') = 672;
    mInterval('schedule', 'stepsPerInterval', 'c001') = 4;
    mInterval('schedule', 'lastStepInIntervalBlock', 'c001') = 1344;
...
* --- Define Unit Approximations ----------------------------------------------
    ...
    mSettingsEff('schedule', 'level1') = 36;
...
*    mSettingsEff('schedule', 'level3') = 8760;
    ...
    mSettings('schedule', 't_trajectoryHorizon') = 36;
