...
*nnu(node, node_, unit)$(nu(node, unit) and ord(node) = ord(node_)) = yes; // NOT USED ATM
...
*unit_withConstrainedOutputRatio(unit)$(sum(gngnu_constrainedOutputRatio(grid, node, grid_, node_, unit), 1)) = yes; // NOT USED ATM
...
*node_to_node(from_node, to_node)$p_gnn('elec', from_node, to_node, 'transferCap') = yes; // NOT USED ATM
