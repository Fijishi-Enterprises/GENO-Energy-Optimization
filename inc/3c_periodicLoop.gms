...
loop(cc(counter),
    ...
    loop(ms(mSolve, s),
        ...
        if(mInterval(mSolve, 'stepsPerInterval', counter) = 1,
            ...
            //external forced generation constraint
            ts_vgen_fixed_(gn(grid, node), unit, ft(f_solve, tt_interval(t))) = ts_vgen_fixed(grid, node, unit, f_solve, t+dt_circular(t));
        ...
        elseif mInterval(mSolve, 'stepsPerInterval', counter) > 1,
            ...
            loop(ft(f_solve, tt_interval(t)),
                ...
                ts_fuelPrice_(fuel, t)
                 ...
                 //external forced generation constraint
                 ts_vgen_fixed_(gn(grid, node), unit, f_solve, t)
                         = sum(tt(t_), ts_vgen_fixed(grid, node, unit, f_solve, t_+dt_circular(t)))
                           / mInterval(mSolve, 'stepsPerInterval', counter);
