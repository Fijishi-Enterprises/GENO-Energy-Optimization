...
loop(ft(f, t),
    ...
    // v_state absolute boundaries set according to gnData parameters;
    ...
    // v_stateSlack absolute boundaries determined by the slack data in pgnData
    v_stateSlack.up(pgn(slack, inc_dec, grid, node), f, t)$pgnData(slack, inc_dec, grid, node, 'maxSlack') = pgnData(slack, inc_dec, grid, node, 'maxSlack');
