...
v_state.fx(gn_state(grid, node), f, t)$(mftLastSteps(mSolve, f, t) and p_gn(grid, node, 'fixState')) = p_gn(grid, node, 'fixState') * ( p_gn(grid, node, 'maxState')${not p_gn(grid, node, 'absolute')} + p_gn(grid, node, 'absolute') );
