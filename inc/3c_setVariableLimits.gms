...
v_gen.up(gnu(grid, node, unit), ft(f, t))$(not unit_VG(unit)) = p_gnu(grid, node, unit, 'maxCap') * sum(node_, p_nu(node_, unit, 'availability'));
v_gen.up(gnu(grid, node, unit_VG), ft(f, t))$(not unit_hydro(unit_VG))      // Should only be about reservoir or RoR+reservoir hydro
...
v_gen.lo(gnu(grid, node, unit), ft(f, t))$(not p_gnu(grid, node, unit, 'maxCharging')) = 0;
// Max. consumption capacity for chargable storages
v_gen.lo(gnu_input(grid, node, unit), ft(f, t)) = -p_gnu(grid, node, unit, 'maxCharging');
// Max. consumption for units that convert endogenous inputs to endogenous outputs
v_gen.lo(gnu_input(grid_, node_input, unit), ft(f, t)) = sum(gn(grid, node)$nu(node, unit), -p_gnu(grid, node, unit, 'maxCap') * p_nu(node, unit, 'slope'));
// Fixed generation for run-of-river
v_gen.up(gnu('elec', node, unit), ft(f, t))$unitFuelParam(unit, 'water', 'main') = ts_inflow_(unit, f, t);

// v_online cannot exceed 1
v_online.up(nu(node, unit_online), ft(f, t)) = 1;
...
// Max. & min. spilling
v_spill.lo(gnStorage(grid, node, storage), ft(f, t))
    = p_gnStorage(grid, node, storage, 'minSpill') * p_stepLength(mSolve, f, t);
v_spill.up(gnStorage(grid, node, storage), ft(f, t))
...
v_resTransCapacity.up(restype, 'resUp', from_node, to_node, ft(f, t))$(restypeDirectionNode(restype, 'resUp', from_node) and restypeDirectionNode(restype, 'resUp', to_node))
...
v_reserve.up(nuRescapable(restype, 'resUp', node, unit_elec), ft(f, t))
...
v_reserve.up(nuRescapable(restype, 'resDown', node, unit_elec), ft(f, t))
