...
$endif
    ...
    if(tSolveFirst = mSettings('schedule', 't_start') and not active('addOn'),
        v_online.up(nuft(node, unit_online, f, tSolve)) = 1;
    ...
    // Fix online capacity and state for later solutions
        ...
        v_online.fx(nuft(node, unit_online, f, tSolve)) = v_online.l(node, unit_online, f, tSolve);
...
v_gen.up(gnuft(grid, node, unit, f, t))$(not unit_VG(unit)) = p_gnu(grid, node, unit, 'maxCap') * sum(node_, p_nu(node_, unit, 'availability'));
v_gen.up(gnuft(grid, node, unit_VG, f, t))      // Should only be about variable generation
...
v_gen.lo(gnuft(grid, node, unit, f, t))$(not p_gnu(grid, node, unit, 'maxCharging')) = 0;
// Max. consumption capacity for chargable storages
v_gen.lo(gnuft(grid, node, unit, f, t))$gnu_input(grid, node, unit) = -p_gnu(grid, node, unit, 'maxCharging');
// Max. consumption for units that convert endogenous inputs to endogenous outputs
v_gen.lo(gnuft(grid_, node_input, unit, f, t))$gnu_input(grid_, node_input, unit) = sum(gn(grid, node)$nu(node, unit), -p_gnu(grid, node, unit, 'maxCap') * p_nu(node, unit, 'slope'));

// v_online cannot exceed 1
v_online.up(nuft(node, unit_online, f, t)) = 1;
// Restrict v_online also in the last dynamic time step
v_online.up(nuft(node, unit_online, f, t))$mftLastSteps(mSolve,f,t) = 1;
